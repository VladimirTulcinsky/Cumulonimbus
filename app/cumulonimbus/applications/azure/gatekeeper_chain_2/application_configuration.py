...
class ApplicationConfiguration(ApplicationConfigurationAbstract):
    ...
    def get_hints(self) -> dict:
        return {
            ...
            4: "Later stages need non-Reader roles: App Service settings via Website Contributor (`az webapp config appsettings list`), and the Event Grid full webhook URL via EventGrid Contributor (`az eventgrid event-subscription show --include-full-endpoint-url`). The final unlock grants Key Vault Secrets User: `az keyvault secret show --vault-name <kv> --name app-flag --query value -o tsv` (the vault name/secret are in the Event Grid topic's tags).",
        }
