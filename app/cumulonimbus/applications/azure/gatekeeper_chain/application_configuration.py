...
class ApplicationConfiguration(ApplicationConfigurationAbstract):
    ...
    def get_hints(self) -> dict:
        return {
            ...
            4: "The ACR stage needs Docker, which this container does not have — run it on a machine that does (e.g. your own host): `az acr login --name <acr>`, `docker pull <acr>.azurecr.io/cumulonimbus/app:latest`. The runtime file /app/config/app.config is a decoy; the real flag was written into a layer then deleted, so recover it with `docker history --no-trunc <img> | grep deploy_token` (or `docker save` + grep the layers). The final unlock grants Key Vault Secrets User: `az keyvault secret show --vault-name <kv> --name app-flag --query value -o tsv` (the vault name/secret are in the APIM `next-hop` named value).",
        }
