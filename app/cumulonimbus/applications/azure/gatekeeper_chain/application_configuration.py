...
class ApplicationConfiguration(ApplicationConfigurationAbstract):
    ...
    def get_hints(self) -> dict:
        return {
            ...
            3: "The ladder walks the real plaintext-credential scenarios in this order: Container Instance env (`az container show`) -> Data Factory linked service (`az datafactory linked-service show ... --query \"properties.[description, annotations]\"` — the key is in the description, not the encrypted connection string) -> App Configuration (`az appconfig kv list --auth-mode login`) -> Monitor action group (`az monitor action-group show`) -> APIM named value (`az apim nv show ... --named-value-id flag-key`).",
