...
resource "azuread_user" "attacker" {
  user_principal_name   = "kv-attacker${local.name_suffix_dash}@${var.tenant_domain}"
  display_name          = "KV Attacker${local.name_suffix_dash}"
  mail_nickname         = "kv-attacker${local.name_suffix_dash}"
