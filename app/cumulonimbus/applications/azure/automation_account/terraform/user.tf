...
resource "azuread_user" "attacker" {
  user_principal_name   = "aa-attacker${local.name_suffix_dash}@${var.tenant_domain}"
  display_name          = "AA Attacker${local.name_suffix_dash}"
  mail_nickname         = "aa-attacker${local.name_suffix_dash}"
