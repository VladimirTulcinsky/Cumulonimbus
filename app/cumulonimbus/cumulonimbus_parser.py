...
class CumulonimbusParser:
    ...
    def _init_aws_parser(self):
        ...
        aws_auth_params.add_argument('--session-name',
                                     ...
                                     default=None,
                                     dest='name_suffix',
                                     help='Optional per-player identifier used to namespace deployed resources, so multiple users can share one account without collisions. Letters/digits only, max 8 chars; a short random tag is appended so identical names do not collide (e.g. your initials or team name)')
    ...
    def _init_azure_parser(self):
        ...
        azure_auth_s_params.add_argument('--session-name',
                                         ...
                                         default=None,
                                         dest='name_suffix',
                                         help='Optional per-player identifier used to namespace deployed resources, so multiple users can share one tenant without collisions. Letters/digits only, max 8 chars; a short random tag is appended so identical names do not collide (e.g. your initials or team name)')
